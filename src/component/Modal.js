...
const CustomModal = ({ visible, onClose, onLocationSelect }) => {
  ...
  const initialRegion = {
    latitude: -1.831239,
    longitude: -78.183405,
    latitudeDelta: 5, // Valor arbitrario para mostrar una gran área de Ecuador
    longitudeDelta: 5,
  };
  ...
  // Función para manejar la selección de un lugar en el mapa
  ...
  // Función para enviar los datos de ubicación al componente padre
  const handleLocationSelect = () => {
    if (selectedLocation) {
      onLocationSelect(selectedLocation);
      onClose(); // Cerrar el modal después de enviar los datos al padre
    }
  };

  // Efecto para obtener la ubicación actual cuando se abre el modal
  useEffect(() => {
    if (visible) {
  ...
  // Efecto para centrar el mapa en la ubicación seleccionada
  useEffect(() => {
    if (selectedLocation) {
      const newRegion = {
    ...
    >
      <View style={styles.modalBackground}>
        <View style={styles.modalContainer}>
          ...
          {visible && ( // Mostrar el botón solo cuando el modal es visible
...
const styles = StyleSheet.create({
  ...
  locationButton: {
    ...
    borderRadius: 30,
    padding: 10,
    zIndex: 999, // Asegurar que esté por encima del mapa y otros elementos
  },
  locationDetails: {
    marginTop: 10,
    padding: 10,
